// CalColor3D-NS.cpp : 定义控制台应用程序的入口点。
//非肤色直方图
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	file.open("E:/FaceDetection/non-skin-list-good");
	...
	while(file.getline(st,20,'\n'))
	{	
		char *path=(char*)malloc(strlen(st)+33);//将从list中读出的文件名拼接成图片文件的物理地址
				
		strcpy(path,"E:/FaceDetection/non-skin-images/");
		strcat(path,st);
	...
	src= cvLoadImage(path);//文件路径要用/而不是\，否则会被误认为是转义字符
	...
	{
		printf("%d:",count);
	cvCalcHist(planes,hist,1,0); //一定要先初始化hist，也就实现建立好一个已进行过统计的直方图之后再进行迭代才可以
	}
...
//	}
	...
	CvFileStorage* fs=cvOpenFileStorage("D:/projects/non-skin-data.xml",0,CV_STORAGE_WRITE);
	cvWriteReal(fs,"total_count",bin_val);
	cvWrite(fs,"Non-Skin_Color_Histogram",hist);
